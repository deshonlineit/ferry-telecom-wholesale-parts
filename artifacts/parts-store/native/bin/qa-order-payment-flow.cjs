...
assert.match(checkout, /const codes = \['stripe', 'twint', 'pay_later'\]/);
...
assert.match(checkout, /app-wallee\\\.com/);
...
assert.match(commerce, /\['swiss_qr_invoice', 'twint'\]/);
assert.match(commerce, /commerceResolvePayLaterMethod/);
assert.match(commerce, /foreach \(\['stripe', 'twint', 'pay_later'\] as \$method\)/);
...
assert.match(account, /swiss_qr_invoice/);
assert.match(account, /pay_later/);
