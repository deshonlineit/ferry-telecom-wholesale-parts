            ...
            root.innerHTML = `<div data-catalog-shell><div class="catalog-breadcrumb"><a href="${window.APP_BASE}">${t('home')}</a><span>/</span><a href="${D.buildUrl('')}">${t('catalogue')}</a>${cat ? `<span>/</span><span>${escape(cat.name)}</span>` : ''}</div>
                ...
                <div class="catalog-layout">
                    <aside class="catalog-sidebar" aria-label="${t('browseFilterCatalogue')}">
                        ...
                        <nav class="quick-categories" aria-label="${t('choosePartCategory')}"><a class="quick-category ${!cat ? 'active' : ''}" ${!cat ? 'aria-current="page"' : ''} href="${D.buildUrl(params, {category: ''})}">${D.railGlyph()}<span>${t('allParts')}</span></a>${quickCategories.map(c => `<a class="quick-category ${c.id === cat?.id ? 'active' : ''}" ${c.id === cat?.id ? 'aria-current="page"' : ''} href="${D.buildUrl(params, {category: c.id, part: ''})}">${D.categoryThumb(c)}<span>${escape(window.I18n.dictionaries.en[c.slug] ? t(c.slug) : quickNames[c.slug] || c.name)}</span><small>${window.I18n.number(c.count)}</small></a>`).join('')}</nav>
                        ${typePicker}
                    ...
                    <section class="catalog-main" data-catalog-results tabindex="-1" aria-label="Product results" aria-busy="false"><p class="catalog-result-status sr-only" aria-live="polite">${window.I18n.number(result.total)} ${t(result.total === 1 ? 'part' : 'parts')}</p>
                        ...
                        <div class="catalog-toolbar"><span class="result-range">${result.total ? t('resultRange', {from: window.I18n.number((page - 1) * (Number(params.get('limit')) || 24) + 1), to: window.I18n.number(Math.min(page * (Number(params.get('limit')) || 24), result.total)), total: window.I18n.number(result.total)}) : t('noResults')}</span>
