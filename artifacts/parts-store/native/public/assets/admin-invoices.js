        ...
        dialog.innerHTML = `
            ...
            <form id="invoice-payment-form">
                ...
                <div class="invoice-form-grid">
                    <div class="form-group"><label for="invoice-paid-amount">Total amount received (${esc(invoice.currency || 'CHF')})</label><input class="form-control" id="invoice-paid-amount" name="paid_amount" type="number" inputmode="decimal" step="0.01" min="0" max="1000000" value="${(Number(invoice.paid_cents) / 100).toFixed(2)}" required aria-describedby="invoice-amount-help"><small id="invoice-amount-help">The total you are confirming, not an additional payment.</small><button type="button" class="btn btn-sm btn-outline" data-invoice-fill-paid style="margin-top: 0.5rem;">Enter full amount received</button></div>
                    <div class="form-group"><label for="invoice-due-date">Due Date</label><input class="form-control" id="invoice-due-date" name="due_date" type="date" value="${esc(invoice.due_date || '')}"><small>Leave blank if no due date has been set.</small></div>
                </div>
                ...
                <div class="form-group"><label for="invoice-note">Reason for this Change</label><textarea class="form-control" id="invoice-note" name="note" rows="2" minlength="5" maxlength="1000" required placeholder="For example: bank receipt checked, reference …"></textarea><small>The change and explanation are recorded in the administration audit log.</small></div>
    ...
    window.Router.add(/^admin\/invoices$/, async (match, root, params) => {
        ...
        root.innerHTML = window.Admin.layout(`
            <section class="admin-finance">
                ...
                <form id="invoice-filter-form" class="admin-toolbar" style="margin-bottom:1.5rem;">
                    <div class="form-group"><label for="invoice-search">Search</label><input class="form-control" id="invoice-search" name="q" type="search" maxlength="160" placeholder="Order number, company or email…" value="${esc(params.get('q') || '')}"></div>
                    <div class="form-group"><label for="invoice-status">Payment Status</label><select class="form-control" id="invoice-status" name="status">${Object.entries(labels).map(([value, label]) => `<option value="${value}" ${status === value ? 'selected' : ''}>${label}</option>`).join('')}</select></div>
                    <div class="form-group"><label for="invoice-sort">Sort</label><select class="form-control" id="invoice-sort" name="sort">${[['newest', 'Newest first'], ['oldest', 'Oldest first'], ['due', 'Due date'], ['amount_desc', 'Highest amount']].map(([value, label]) => `<option value="${value}" ${sort === value ? 'selected' : ''}>${label}</option>`).join('')}</select></div>
                    <div class="form-group"><label for="invoice-limit">Per Page</label><select class="form-control" id="invoice-limit" name="limit">${[25, 50, 100].map(value => `<option value="${value}" ${Number(params.get('limit') || 50) === value ? 'selected' : ''}>${value}</option>`).join('')}</select></div>
                    <button class="btn btn-primary" type="submit" style="flex:0 0 auto">Apply</button><a class="btn btn-outline" href="${window.APP_BASE}admin/invoices" style="flex:0 0 auto">Clear</a>
                </form>
