...
function handleCatalog(string $method, string $path): bool
{
    ...
    if (preg_match('#^/products/(\d+)$#', $path, $match)) {
        $query = db()->prepare("SELECT * FROM products WHERE id=? AND active=TRUE AND publication_status='visible'");
        ...
        $query = db()->prepare("SELECT * FROM products WHERE active=TRUE AND publication_status='visible' AND category_id=? AND id<>? ORDER BY featured DESC,stock>0 DESC LIMIT 4");
