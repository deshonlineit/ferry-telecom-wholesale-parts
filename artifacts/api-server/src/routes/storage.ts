...
router.get('/storage/objects/*path', async (req: Request, res: Response) => {
  try {
    ...
    // Publicly-visible objects (ACL visibility "public") are served to anyone;
    // everything else requires an authenticated user that passes the ACL.
    ...
    if (!canAccess) {
      res.status(userId ? 403 : 401).json({ error: 'Unauthorized' });
      return;
    }
