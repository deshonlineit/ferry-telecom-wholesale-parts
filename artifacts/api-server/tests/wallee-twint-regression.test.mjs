...
const [wallee, stripe, commerce, operations, store] = await Promise.all([
  readFile(new URL("../src/routes/wallee.ts", import.meta.url), "utf8"),
  readFile(new URL("../src/routes/stripe.ts", import.meta.url), "utf8"),
...
assert.match(stripe, /payment_method_types:\s*\["card"\]/, "Stripe must remain card-only");
