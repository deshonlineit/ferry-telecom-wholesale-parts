...
Clean up the entire cart owned by a synthetic test customer, not just cart lines for newly created test products. Keep deletion of catalog products limited to the exact fixtures created for that run.

**Why:** Browser ordering checks can add both a temporary SKU and an existing SKU. A remaining cart reference can prevent deleting the synthetic customer and roll back the entire cleanup transaction. Older QA-labelled catalog records may also be part of the retained baseline, not disposable leftovers.
