...
An active catalogue product without an assigned EUR base price remains visible
in catalogue, model, and search listings but is individually unorderable. It
must not make commerce globally unavailable. Correctly priced products and
empty carts must continue to work.
...
**How to apply:** publication does not require a complete price set. Catalogue
listings expose an individually missing price as null and keep rendering the
product and its neighbours. Keep strict null-price rejection at cart entry,
cart-item pricing, quote, and checkout boundaries. Do not derive global commerce
readiness from a count of all active products with null prices.
