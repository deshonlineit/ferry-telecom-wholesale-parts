...
**How to apply:** When development is the approved source of truth, make data corrections in the development PostgreSQL custom schema—not only in the isolated local MySQL QA copy. Enable “Copy your development database to production database” in the deployment database settings, then publish. This overwrites all current production contents, so obtain informed approval first. For later custom-schema changes, use the copy option again or apply them manually. After promotion, query production and exercise a real data-backed page.