...
- The owner wants a restrained, Apple.com-like storefront while retaining Ferry Telecom branding. Model lookup should offer a short, relevant selection as they type, not a wall of devices, extra brand-selection steps or a panel that repeatedly needs reopening.
...
- Preserve the owner's existing pricing workflow: purchase costs and selling/group prices are set in EUR; Swiss customers order in CHF using the existing conversion calculation, while customers abroad order in EUR. Staff enter normal decimal currency amounts, never raw cents. **Why:** the owner explicitly rejected the CHF-first test implementation and wants business parity, not a redesigned pricing model.
- Verify the existing exchange-rate source, calculation, rounding and country-selection rule before implementing currency conversion. Do not guess a rate or silently relabel existing CHF prices or historical order/document amounts as EUR.
